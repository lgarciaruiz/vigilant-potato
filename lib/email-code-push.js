...
export default {
  ...
  activate(state) {
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      ...
      'email-code-push:toggle-prod': () => this.promptUserConfirm('prod')
    }));
  },
  ...
  errorLog(error, env) {
    ...
    if (error.response) {
      ...
      if (error.response.statusText === 'Unauthorized') {
        if (env === 'prod') {
          this.token_prod.expired = true;
        } else {
          this.token_test.expired = true;
        }
    ...
    } else if (error.message === 'abortProcess') {
      message = 'CANCELLED PUSH TO PROD'
    } else {
    ...
    atom.notifications.addError('Code not pushed', options);
  ...
  async authenticate(env) {
    ...
    if (env === 'prod') {
      console.log('setting creds for prod');
      authUrl = this.credentials.auth_url_prod;
      clientID = this.credentials.client_id_prod;
      clientSecret = this.credentials.client_secret_prod;
    };

    if (!authUrl) {
      atom.notifications.addError('ERROR IN PACKAGE SETTINGS! Update the settings and retry.', {
        detail: `NO AUTH URL FOUND for ${env.toUpperCase()}`,
        dismissable: true
      });
    } else if (!clientID) {
      atom.notifications.addError('ERROR IN PACKAGE SETTINGS! Update the settings and retry.', {
        detail: `NO CLIENT ID FOUND for ${env.toUpperCase()}`,
        dismissable: true
      });
    } else if (!clientSecret) {
      atom.notifications.addError('ERROR IN PACKAGE SETTINGS! Update the settings and retry.', {
        detail: `NO CLIENT SECRET FOUND for ${env.toUpperCase()}`,
        dismissable: true
      });
    } else {
  ...
  getCurrFileName(){
    if (editor = atom.workspace.getActiveTextEditor()) {
      return parseFileName(editor.getTitle());
    }
  },

  promptUserConfirm(env) {

    const environment = env;

    //set up buttons to ask user for confirmation
    const buttons = {
      buttons: [{
        ...
        },
        {
          ...
          text: 'Cancel',
          onDidClick: () => {
            let error = new Error('abortProcess');
            this.errorLog(error);
          }
        }
      ],
    ...
    let fileName = this.getCurrFileName();
    //set notification equal to alert to dismiss when button pressed
    this.alert = atom.notifications.addInfo(`WARNING Pushing "${fileName}" to PROD Environment.`, buttons);
  ...
  async toggle(env) {
    ...
    //console.log('authorization complete', token.token);
    ...
    if (editor = atom.workspace.getActiveTextEditor()) {
      ...
      let assetID;
      let assetReturned;
      const fileName = this.getCurrFileName();
      ...
      if (authToken) {
        ...
        axios(config)
          .then(async (response) => {
            ...
            if (count > 1) {
              ...
              for (let i = 0; i < assetReturned.items.length; i++) {
                ...
                //console.log(assetReturned.items[i]);
          ...
          .then((response) => {
            ...
            atom.notifications.addSuccess(`Code has been pushed into ${env.toUpperCase()} SMC file: ${response.data.name}`);
          })
          .catch((error) => {
